...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6 fade-in">
      <PageHeader title="Admin Dashboard" subtitle="Company-wide petty cash overview" />

      {/* Global Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard title="Total Sites" value={String(stats?.total_sites ?? sites.length)} borderColor="border-amber-400"
        ...
        <StatsCard
          title="Total Issued"
          value={formatCurrency(stats?.total_cash_issued || 0)}
      ...
      {loading ? <Loader /> : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {stats && (
            <BalanceCard balance={stats.balance} totalIssued={stats.total_cash_issued || 0} totalSpent={stats.total_spent || 0} />
          )}
          {Object.keys(categoryTotals).length > 0 && <CategoryChart data={categoryTotals} />}
        </div>
      )}
    </div>
  );
