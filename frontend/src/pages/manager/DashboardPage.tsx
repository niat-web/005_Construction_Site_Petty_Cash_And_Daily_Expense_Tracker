...
export default function SupervisorDashboardPage() {
  ...
  const todayExpenses = expenses.filter((expense) => expense.expense_time.startsWith(today));
  const todayCategoryTotals = todayExpenses.reduce<Record<string, number>>((totals, expense) => {
  ...
  return (
    <div className="space-y-6 fade-in">
      <PageHeader
        title="My Dashboard"
        subtitle={stats?.site_name ? `Today's petty cash summary for ${stats.site_name}` : "Today's petty cash summary for your site"}
      />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard
          title="Today Issued"
          value={formatCurrency(stats?.issued_today || 0)}
        ...
        <StatsCard
          title="Today Spent"
          value={formatCurrency(stats?.spent_today || 0)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {stats && (
          <BalanceCard
            ...
            totalIssued={stats.issued_today || 0}
            totalSpent={stats.spent_today || 0}
          />
        )}
        {Object.keys(todayCategoryTotals).length > 0 && <CategoryChart data={todayCategoryTotals} />}
      </div>

      {/* Recent Expenses */}
      <div>
        <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">Recent Expenses</h2>
        <ExpenseTable expenses={todayExpenses.slice(0, 5)} showActions={false} />
      </div>
    </div>
  );
}
